...
class SearchAssisten():
    def __init__(self, documents=None, config=None) -> None:
    ...
    def querySerpapi(self, query: str, limit: int = 5, do_analysis: bool = True, engine="google") -> str:
    ...
    def scraping_call(self, query, search_method="serpapi"):
        if search_method == "serpapi":
            _, links = self.querySerpapi(query, do_analysis=True)
        elif search_method == "serper":
            _, links = self.querySerper(query, do_analysis=True)
        elif search_method == "openserp":
            _, links = self.queryOpenserp(query, do_analysis=True)
        self.documents, links = self.web_crawl(urls=links)
