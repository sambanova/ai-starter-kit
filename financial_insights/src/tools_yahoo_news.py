...
def filter_text(text: str) -> List[str]:
    ...
    if len(text.split()) >= 4 and len(text.split()) <= MAX_CHUNK_SIZE:
        ...
        splitter = CharacterTextSplitter(chunk_size=MAX_CHUNK_SIZE, chunk_overlap=64, separator=r'[.!?]', is_separator_regex=True)
...
def scrape_yahoo_finance_news(ticker_list: List[str]) -> None:
    """
