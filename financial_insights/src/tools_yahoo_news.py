...
def scrape_yahoo_finance_news(
    ticker_list: List[str], user_request: str
) -> Tuple[str, List[str]]:
    """
        ...
        if link_response.status_code == 200:
            ...
            if RETRIEVE_HEADLINES:
                ...
                headline_elements = soup.find_all(
                    attrs={"data-test-locator": "headline"}
                )
                ...
                for headline_element in headline_elements:
                    ...
                    for filtered_text in filtered_texts:
                        data.append(
                            {"type": "h1", "url": link_url, "text": filtered_text}
                        )
                ...
                for h2_tag in soup.find_all("h2"):
                    ...
                    for filtered_text in filtered_texts:
                        data.append(
                            {"type": "h2", "url": link_url, "text": filtered_text}
                        )
                ...
                for h3_tag in soup.find_all("h3"):
                    ...
                    for filtered_text in filtered_texts:
                        data.append(
                            {"type": "h3", "url": link_url, "text": filtered_text}
                        )
            ...
            if link_url in singular_urls:
                ...
                for p_tag in soup.find_all("p"):
                    ...
                    for filtered_text in filtered_texts:
                        data.append(
                            {"type": "p", "url": link_url, "text": filtered_text}
                        )
    ...
    df = pandas.DataFrame(
        {"News": headlines_list + h2_headings_list + h3_headings_list + paragraphs_list}
    )
