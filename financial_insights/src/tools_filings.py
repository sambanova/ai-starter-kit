...
class SecEdgarFilingsInput(BaseModel):
    """Retrieve the text of a financial filing from SEC Edgar to answer the user query."""

    user_query: str = Field('The user query.')
    # url_filing: str = Field(
    #     description='The URL to the SEC Edgar filing page.',
    # )
    ticker_symbol: str = Field('The company ticker symbol.')
    filing_type: str = Field('The type of filing (between "10-K" and "10-Q").')
...
def retrieve_filings(user_query: str, ticker_symbol: str, filing_type: str, filing_quarter: int, date: int) -> Optional[str]:
    """Retrieve the text of a financial filing from SEC Edgar to answer the user query."""
    ...
    if filing_type == '10-Q':
        if filing_quarter is not None:
            ...
            assert filing_quarter in [1,2,3,4], 'The quarter must be between 1 and 4.'
    ...
    for metadata in metadatas: 
        ...
        if report_date.year == date:
            if filing_quarter == 0 or (filing_quarter is not None and (report_date.month - filing_quarter*3) <= 1):
                ...
                splitter = CharacterTextSplitter(chunk_size=512, chunk_overlap=64, separator=r'[.!?]', is_separator_regex=True)
    ...
    if filing_type == '10-K':
        ...
        raise Exception(f"Filing document {filing_type} for  {ticker_symbol} for the quarter {filing_quarter} of the year {date} is not available")
