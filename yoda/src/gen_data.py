...
def pretrain(articles, data_dir: str, processed_data_folder: str = 'processed_data',
             output_file_name: str = 'article_data.jsonl') -> list:
    article_data = []
    for d in articles:
        article = d["article"]
        # clean the article
    ...
    output_path = os.path.join(
        data_dir, processed_data_folder, output_file_name)
...
def finetune(articles, data_dir, tokenizer, pretrain_data=None, n_eval_samples = 50) -> None:
    ...
    for doc in tqdm.tqdm(articles):
        ...
        response_text, prompt_length = qa_processing.generate_qa_pairs(
            sample=doc, template=QA_GEN_TEMPLATE,
        ...
        with jsonlines.open(os.path.join(data_dir, "response_data", "llama2_70b_chat_qa_gen_responses.jsonl"),
                            mode='a') as writer:
    ...
    response_file = os.path.join(
        data_dir, "response_data", "llama2_70b_chat_qa_gen_responses.jsonl")
    qa_train_file = os.path.join(
        data_dir, "processed_data", "synthetic_qa_train.jsonl")
    ...
    qa_data, need_to_regenerate = qa_processing.process_response_data(
        response_data)
    ...
    qa_article_mix_file = os.path.join(
        data_dir, "processed_data", "qa_article_mix.jsonl")
    if pretrain_data:
