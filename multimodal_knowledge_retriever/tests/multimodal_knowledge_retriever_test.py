...
class MKRTestCase(unittest.TestCase):
    ...
    def parse_data(cls):
        ...
        text_docs, table_docs, image_paths = cls.multimodal_retriever.process_raw_elements(raw_pdf_elements, [IMAGE_TEST_DATA_PATH])
    ...
    def create_vectorstore(cls):
        ...
        return cls.multimodal_retriever.vectorstore_ingest(retriever, cls.text_docs, cls.table_docs, cls.image_paths, summarize_texts=True, summarize_tables=True)
    ...
    def test_document_parsing(self):
        self.assertTrue(len(self.text_docs) > 0, "There should be at least one parsed chunk")
        self.assertTrue(len(self.table_docs) > 0, "There should be at least one parsed chunk")
