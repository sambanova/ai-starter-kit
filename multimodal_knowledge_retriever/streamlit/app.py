...
def handle_user_input(user_question):
    if user_question:
        ...
        # List of sources
        sources =set([
            f'{sd.metadata["filename"]} {" - Page "+str(sd.metadata.get("page_number")) if sd.metadata.get("page_number")else " - "+sd.metadata["file_directory"].split("/")[-1]}'
            for sd in response["source_documents"]
        ])
        image_sources = [os.path.join(sd.metadata["file_directory"], sd.metadata["filename"]) for sd in response["source_documents"] if sd.metadata["filename"].endswith((".png",".jpeg",".jpg"))]
...
def main():
    ...
    with st.sidebar:
        ...
        docs = st.file_uploader(
                "Add your files", accept_multiple_files=True, type=["pdf", "jpg", "jpeg", "png"]
            )
        ...
        st.caption("**Note** If selected the kit will use raw images to generate the answers, if not, image summaries will be used instead")
