                ...
                with st.spinner('Processing...'):
                    try:
                        response = st.session_state.fc.function_call_llm(
                            query=user_question,
                            max_it=st.session_state.max_iterations,
                            debug=True)

                        # If we get here, the function call was successful
                        break
                        ...
                        if "database is locked" in str(e):
                            ...
                            raise  # re-raise if it's a different sqlite3.OperationalError
            except Exception as e:
                st.error(f"An unexpected error occurred: {str(e)}")
                return
        else:
            # This will only execute if the loop completed without breaking
            st.error(
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.markdown(
            "Get your SambaNova API key [here](https://sambanova.ai/fast-api)")
        ...
        if not are_credentials_set(additional_env_vars):
            ...
            if st.button('Clear Credentials'):
                save_credentials(
                    '', {var: ''
                         for var in (additional_env_vars or [])}, prod_mode)
                st.rerun()
        ...
        if are_credentials_set(additional_env_vars):
            ...
            st.markdown(
                '**2. Set the maximum number of iterations you want the model to run**'
            )
            st.session_state.max_iterations = st.number_input('Max iterations',
                                                              value=5,
                                                              max_value=20)
            st.markdown(
                '**Note:** The response may not be completed if the max number of iterations is too low'
            )
            if st.button('Set'):
                with st.spinner('Processing'):
                    set_fc_llm(st.session_state.tools)
                    st.toast(
                        f'Tool calling assistant set! Go ahead and ask some questions',
                        icon='🎉')
            ...
            with st.expander('Additional settings', expanded=False):
                ...
                st.markdown(
                    '**Note:** Resetting the chat will clear all interactions history'
                )
                if st.button('Reset conversation'):
                    st.session_state.chat_history = []
                    st.session_state.sources_history = []
                    global_output.truncate(0)
                    global_output.seek(0)
                    st.toast(
                        'Interactions reset. The next response will clear the history on the screen'
                    )
    ...
    # Update the scratchpad after processing
    with st.sidebar:
        with st.expander('**Execution scratchpad**', expanded=True):
            st.code(global_output.getvalue())
