...
def get_stock_info(
    ...
    if not isinstance(user_query, str):
        raise TypeError(f'User query must be of type string. Got {(type(user_query))}.')
    ...
    if not isinstance(dataframe_name, str | None):
        raise TypeError(f'Dataframe name must be a string. Got {(type(dataframe_name))}.')
...
def get_yahoo_connector_answer(user_query: str, symbol: str) -> Any:
    ...
    # Checks the inputs
    if not isinstance(user_query, str):
        raise TypeError(f'The user query must be of type string. Got {(type(user_query))}.')
    if not isinstance(symbol, str):
        raise TypeError(f'The company symbol must be of type string. Got {(type(symbol))}.')
...
def retrieve_symbol_list(company_names_list: List[str] | str = list()) -> List[str]:
    ...
    Raises:
        ...
        ValueError: If `company_names_list` does not contain any company names.
    """
    # Check inputs
    if not isinstance(company_names_list, (list, str)):
        raise TypeError(f'company_names_list` names must be a list of strings. Got {type(company_names_list)}.')

    # If `symbol_list` is a string, coerce it to a list of strings
    company_names_list = coerce_str_to_list(company_names_list)

    if len(company_names_list) == 0:
        ValueError('`company_names_list` cannot be empty.')

    if not all([isinstance(name, str) for name in company_names_list]):
        raise TypeError('`company_names_list` must be a list of strings.')
...
def get_historical_price(
    ...
    Raises:
        TypeError: If `company_list` is not a string or a list of strings.
    """
    if not isinstance(company_list, (list, str)):
        raise TypeError(f'`company_list` must be of type list or string. Got {(type(company_list))}.')
