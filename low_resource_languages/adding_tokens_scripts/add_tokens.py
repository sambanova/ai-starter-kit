...
if __name__ == "__main__":
    ...
    parser.add_argument('--sp_model_file', default=None, type=str,
                        help='Path to sentence piece tokenizer model file to add to base_tokenizer')
    parser.add_argument('--output_dir', default=None, type=str, help='Path to save the finalized output tokenizer to ')
    args = parser.parse_args()

    merging_sp_model_file = args.sp_model_file
    merging_sp_model = spm.SentencePieceProcessor()
    merging_sp_model.Load(merging_sp_model_file)

    llama_tokenizer = LlamaTokenizer.from_pretrained(args.sp_model_file)  # I've added this myself
    ...
    output_hf_dir = os.path.join(args.output_dir, 'hf_tokenizer')
    os.makedirs(output_hf_dir, exist_ok=True)
    output_sp_model_path = args.output_dir + '/sentence_piece_tokenizer.model'
