...
from utils.visual.env_utils import are_credentials_set, env_input_fields, save_credentials, initialize_env_variables
    ...
    with st.sidebar:
        ...
        if are_credentials_set(additional_variables):
            ...
            with st.expander("#### 3. Define the model output tools parser to use", expanded=True):
                available_parsers = ["JSON tools parser (llama custom)", "XML tools parser (deepseek custom)", "Custom defined tools parser"]
                    ...
                    if st.button("Set parser", key="set_parser_button"):
                        if "llama" in parser_source.lower():
                            ...
                            with st.popover("**JSON parser (llama custom)** `src`"):
                                st.code(app_description.get("llama_json_parser_ref_code"), language="python", height=300)
                            ...
                            with st.popover("**XML parser (deepseek custom)** `src`"):
                                st.code(app_description.get("deepseek_xml_parser_ref_code"), language="python", height=300) 
...
def main_interaction_area(app_description: dict[str, str]):
        ...
        for i, msg in enumerate(st.session_state.messages):
            ...
            with role_col:
                st.session_state.messages[i]["role"] = st.selectbox(
                    f"Role {i+1}", ["system", "user", "assistant", "tool"], index=["system", "user", "assistant", "tool"].index(msg["role"]),
            ...
            with content_col:
                if st.session_state.messages[i]["role"]=="assistant":
                    ...
                    if not st.session_state.messages[i]["tool_calls"]:
                        ...
                        st.session_state.messages[i]["tool_calls"]=json.loads(st.session_state.messages[i]["tool_calls"])   
    ...
    with invoke_col: 
        if st.button(f"Invoke {('ˋ'+st.session_state.api_model_name+'ˋ') if st.session_state.api_model_name is not None else ''}", key="invoke_model_btn", type="primary"):
    ...
    with(parse_col):
        if st.button(f"Parse raw output {('with ˋ'+st.session_state.parser_name+'ˋ parser') if st.session_state.parser_name is not None else ''}"):
                ...
                if st.session_state.raw_response:
                    with st.spinner("parsing"):
                        st.session_state.parsed_output = st.session_state.manager.parse_to_message(
                            ...
                            parser_name=st.session_state.parser_name
                        )
