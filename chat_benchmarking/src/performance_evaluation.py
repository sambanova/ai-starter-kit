...
from chat_benchmarking.src.llmperf.llmperf_utils import LLMPerfResults, flatten, get_tokenizer
from chat_benchmarking.src.llmperf.models import LLMResponse, RequestConfig
...
class BasePerformanceEvaluator(abc.ABC):
    ...
    def adjust_to_exact_tokens(self, text: str, target_token_count: int) -> str:
        ...
        tokenized_text_directory = f'{kit_location}/../../scratch/chat_benchmarking/prompts'
...
class CustomPerformanceEvaluator(BasePerformanceEvaluator):
    ...
    def create_output_filename(self) -> str:
        """Utility for creating a unique filename for a custom chat_benchmarking experiment with a dataset.
    ...
    def stop_benchmark(self) -> None:
        """Stops the chat_benchmarking process by setting the stop event."""
        self.stop_event.set()
        logger.info('chat_benchmarking process has been stopped.')
    ...
    def run_benchmark(
        ...
        # Save chat_benchmarking results to the specified results directory, it it exists
        if self.results_dir:
    ...
    def get_token_throughput_latencies(
        ...
        if self.stop_event.is_set():
            logger.info('chat_benchmarking process terminated early due to stop signal.')
            return {}, []
...
class SyntheticPerformanceEvaluator(BasePerformanceEvaluator):
    ...
    def create_output_filename(self, num_input_tokens: int, num_output_tokens: int) -> str:
        """Utility for creating a unique filename for a synthetic chat_benchmarking experiment given user specified params.
    ...
    def get_token_throughput_latencies(
        ...
        # Build a metrics summary for the results of the chat_benchmarking run
        results = self.build_metrics_summary(
            metrics=[response.metrics for response in llm_responses],
            start_time=start_time,
            end_time=end_time,
