...
from chat_benchmarking.src.llmperf.llmperf_utils import SAMBANOVA_URL, get_tokenizer
...
class SambaNovaCloudAPI(BaseAPIEndpoint):
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        if self.request_config.api_variables:
            self.base_url = self.request_config.api_variables['API_URL']
            self.api_key = self.request_config.api_variables['API_KEY']
        else:
    ...
    def _get_json_data(self) -> Dict[str, Any]:
        ...
        sampling_params['model'] = self.request_config.model
        sampling_params['max_tokens'] = sampling_params.pop('max_tokens_to_generate')
        sampling_params['temperature'] = sampling_params.pop('temperature')
        sampling_params['top_k'] = sampling_params.pop('top_k')
        sampling_params['top_p'] = sampling_params.pop('top_p')
        sampling_params['repetition_penalty'] = sampling_params.pop('repetition_penalty')
