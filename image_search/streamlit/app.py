        ...
        if st.button("Process"):
            with st.spinner("Processing, this could take several minutes"):
                if os.path.exists(st.session_state.images_path):
                    ...
                    st.session_state.image_search.add_images(paths, images)
                    st.toast(f'vector_db stored in {os.path.join(repo_dir,"/data/vector_db")}')
                    st.session_state.search_disabled = False  
                else:
                    st.toast("the provided image path does not exist")
                
    elif "Use" in datasource:
...
with col1:
    choise = st.radio("Search by:", 
                      ["**Description**", "**Image**"], 
                      captions = ["Write the description of the image you want to search", 
                                  "Upload an image and search similar images"],
...
if choise == "**Description**":
    ...
    if st.button('Search', disabled = st.session_state.search_disabled, type="primary"):
        if search_term:
            with st.spinner("Searching"):
                paths, distances = st.session_state.image_search.search_image_by_text(f"A picture of {search_term}", st.session_state.top_number)
    ...
    if st.button('Search', disabled = st.session_state.search_disabled, type="primary"):
        if uploaded_image is not None:
            with st.spinner("Searching"):
                paths, distances= st.session_state.image_search.search_image_by_image(uploaded_image, st.session_state.top_number)
                search_results=(zip(paths, distances))
        else:
