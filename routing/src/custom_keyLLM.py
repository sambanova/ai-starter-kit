        ...
        **model_params
        ...
        if HAS_SBERT and threshold is not None and embeddings is not None:
            ...
            if out_cluster:
                ...
                selected_embeddings = np.array([embeddings[index].tolist() for index in out_cluster])
                out_cluster_keywords = self.llm.extract_keywords(
                    selected_docs,
                    **model_params
                )
                if isinstance(out_cluster_keywords[0], tuple):
                    out_cluster_keywords = [out_cluster_keywords]
                out_cluster_keywords = {index: words for words, index in zip(out_cluster_keywords, out_cluster)}
                uniq_out_cluster_keywords = [out_cluster_keywords[index] for index in out_cluster_keywords]
            ...
            if in_cluster:
                ...
                if candidate_keywords is not None:
                    ...
                    selected_keywords = None
                selected_embeddings = np.array([embeddings[cluster[0]].tolist() for cluster in clusters])
                in_cluster_keywords = self.llm.extract_keywords(
                    selected_docs,
                    **model_params
                )
                if isinstance(in_cluster_keywords[0], tuple):
                    in_cluster_keywords = [in_cluster_keywords]
                uniq_in_cluster_keywords = []
                uniq_in_cluster_keywords = in_cluster_keywords
                in_cluster_keywords = {
            ...
            uniq_keywords = []
            if out_cluster:
                uniq_keywords.extend(uniq_out_cluster_keywords)
            if in_cluster:
                uniq_keywords.extend(uniq_in_cluster_keywords)
