...
def set_env_variables(url, api_key, additional_vars=None, prod_mode=False):
...
def env_input_fields(additional_env_vars=None):
    ...
    return url, api_key, additional_vars

def are_credentials_set(additional_env_vars=None):
    if additional_env_vars is None:
        additional_env_vars = []

    base_creds_set = bool(st.session_state.FASTAPI_URL and st.session_state.FASTAPI_API_KEY)
...
def save_credentials(url, api_key, additional_vars=None, prod_mode=False):
    set_env_variables(url, api_key, additional_vars, prod_mode)
