...
def load_default_config(config_path="config.yaml"):
...
def display_config(config):
    ...
    st.sidebar.text_input(
        "Path", value=config["eval_dataset"]["path"], key="eval_dataset.path"
    )
    st.sidebar.text_input(
        "Question Column",
    ...
    for i, llm in enumerate(config["llms"]):
        st.sidebar.text_input(
            f"LLM {i+1} Name", value=llm["name"], key=f"llms.{i}.name"
        )
        st.sidebar.text_input(
    ...
    for i, llm in enumerate(config["eval_llms"]):
        st.sidebar.text_input(
            f"Eval LLM {i+1} Name", value=llm["name"], key=f"eval_llms.{i}.name"
        )
        st.sidebar.text_input(
    ...
    st.sidebar.text_input(
        "Location", value=config["vector_db"]["location"], key="vector_db.location"
    )
    ...
    st.sidebar.text_input(
        "Pipeline Class", value=config["pipeline"]["class"], key="pipeline.class"
    )
    st.sidebar.text_input(
        "Vector DB Location",
