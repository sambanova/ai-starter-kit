...
@st.cache_resource
def set_retrieval_comparative_process(config: Any) -> Any:
...
def get_ticker_options() -> Dict[str, Any]:
    ...
    tickers_dict = {
        value['ticker'].lower(): f"{value['ticker']} - {value['title']}" for _, value in company_tickers_dict.items()
    }
    return tickers_dict
...
with st.sidebar:
    ...
    st.markdown('**Note:** Depending on the size of the reports, this could take a few seconds')

    force_reload = st.checkbox(
        'Select if you want to force load', value=False, help="If selected, it'll recreate the data base."
    )

    if st.button('Load'):
        with st.spinner('Loading vector DB...'):
            assert ticker_company_1 is not None and ticker_company_2 is not None
            tickers = sorted([ticker_company_1, ticker_company_2])
            path_suffix = '_'.join(tickers)
            st.session_state.sec_multiturn = set_retrieval_comparative_process(
                config={
