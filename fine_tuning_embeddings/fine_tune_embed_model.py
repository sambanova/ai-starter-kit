...
def finetune(
    ...
    if (
        not force_retrain
        and os.path.exists(model_output_path)
        and os.listdir(model_output_path)
    ):
        logging.info(
            f"Found finetuned model at {model_output_path}. Will use this model without retraining."
        )
        return
    else:
        if not force_retrain:
            logging.info(
                "No finetuned model found or directory is empty. Proceeding with finetuning."
            )
        ...
        finetune_wrapper = FinetuneEngineWrapper(
            train_dataset, val_dataset, model_id, model_output_path
        )
...
def evaluate_all(
    ...
    for model in model_ids:
        if isinstance(model, str):
            ...
            eval_results["model_name"] = model + "_" + name_prefix
            results.append(eval_results)

            # create a mew dataframe for the mean of the hit column
            hit_rate = eval_results["is_hit"].mean()
            ...
            st_results.append(
                pd.DataFrame(
                    {"model_id": [model], "evaluation_result": [eval_st_result]}
                )
            )
...
def evaluate(dataset, embed_model, top_k=5, verbose=False):
    ...
    index = VectorStoreIndex(
        nodes, service_context=service_context, show_progress=verbose
    )
...
def generate_data(args):
    ...
    if os.path.exists(
        args.output_data_directory + "/" + "train_nodes.pkl"
    ) and os.path.exists(args.output_data_directory + "/" + "val_nodes.pkl"):
        logging.info("Loading nodes from .pkl files.")
        train_nodes_pkl = load_nodes_from_pkl(
            args.output_data_directory + "/" + "train_nodes.pkl"
        )
        val_nodes_pkl = load_nodes_from_pkl(
    ...
    if os.path.exists(
        args.output_data_directory + "/" + "train_dataset.json"
    ) and os.path.exists(args.output_data_directory + "/" + "val_dataset.json"):
