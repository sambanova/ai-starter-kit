...
layout_engine = PPStructure(
    recovery=False, layout=True, table=True, ocr=False, show_log=False
...
class PaddleOCRLoader:
    ...
    def __init__(
        ...
        Args:
            ...
            save_intermediate (bool, optional): Whether to save intermediate files for debugging purpouses . Defaults
            to True.
    ...
    def convert_pdf_to_images(self, pdf_file_path: str, output_folder: str = 'data/extraction') -> str:
        """
    ...
    def simple_ocr(self, img_file_path: str, ocr: PaddleOCR = ocr) -> Any:
        """
    ...
    def structured_ocr(self, img_file_path: str, ocr: PaddleOCR = layout_engine) -> Any:
        """
    ...
    def show_paddle_structure_bboxs(self, image_path: str, result: List[Dict[str, Any]], save: bool = False) -> Any:
        """
    ...
    def show_simple_bboxes(
        self, image_path: str, bboxs: List[Any], save: bool = False, tag: Optional[str] = None
    ) -> Any:
        """
    ...
    def get_tables(self, result: List[Dict[str, Any]]) -> Tuple[List[Any], list[Any]]:
        """
    ...
    def save_tables(self, tables: List[Any], path: str, tag: str = 'table', save_html: bool = False) -> str:
        """
    ...
    def get_figures(self, result: List[Dict[str, Any]]) -> Tuple[List[Any], List[Any]]:
        """
    ...
    def save_figures(self, figures: List[Any], path: str, tag: str = 'figure') -> str:
        """
    ...
    def get_equations(self, result: List[Dict[str, Any]]) -> Tuple[List[Any], List[Any]]:
        """
    ...
    def save_equations(self, equations: List[Any], path: str, tag: str = 'equation') -> str:
        """
    ...
    def mask_elements_from_image(
        ...
        if save:
            assert image_path is not None, 'image_path should be a string.'
            path = os.path.join(os.path.dirname(image_path), ''.join(os.path.basename(image_path).split('.')[:-1]))
    ...
    def calculate_intersection_percentage(self, bbox1: List[Any], bbox2: List[Any]) -> Any:
    ...
    def bb_intersect(self, bbox1: List[Any], bbox2: List[Any]) -> Tuple[bool, float]:
        """
    ...
    def merge_bboxes(self, bbox1: List[Any], bbox2: List[Any]) -> List[int | float]:
        """
        ...
        merged_bbox = [min(x1_min, x2_min), min(y1_min, y2_min), max(x1_max, x2_max), max(y1_max, y2_max)]
        return merged_bbox

    def get_content_bboxes(
        self, bboxes: List[Any] | Tuple[List[Any], List[Any]] | List[Any], max_persentage_overlap_allowed: int = 10
        ...
        Returns:
            list: list of bounding boxes
        """
        bbox_indexes_to_drop: bool | List[Any] = True
        while bbox_indexes_to_drop:
            ...
            for i in range(len(bboxes)):  # iterate over all bboxes
                intersected = (
                    False  # initialize flag for markig if the analized bbox get an intersection with any other bbox
                )
                for j in range(1, len(bboxes[i:])):  # iterate over the following bboxes
                    ...
                    if persentage > max_persentage_overlap_allowed:  # if intersects in more than n% of the total area
                        ...
                        new_bboxes.append(
                            self.merge_bboxes(bboxes[i], bboxes[i + j])
                        )  # add the merged bbox to the new bbox list (this will replace the i bbox with the merged one)
                        bbox_indexes_to_drop.append(i + j)  # add the intersected bbox tho the bboxes to drop
                        if (
                            i in bbox_indexes_to_drop
                        ):  # remove the i ocurrence form indexes to remove in case now is stored another merged bbox
            ...
            new_bboxes = [
                element for index, element in enumerate(new_bboxes) if index not in bbox_indexes_to_drop
            ]  # remove all bboxes droped
            bboxes = new_bboxes
        return bboxes

    def expand_bounding_boxes(
        self, bboxes: List[Any], img_size: Tuple[float, float], n: int
    ) -> List[Any] | Tuple[List[Any], List[Any]]:
    ...
    def sort_by_y(self, bounding_boxes: List[Any]) -> List[Any]:
        """
    ...
    def detect_columns(self, bounding_boxes: List[Any]) -> Tuple[List[Any], List[Any]] | Tuple[int, List[Any]]:
        """
    ...
    def order_one_column(self, column_boxes: List[Any]) -> List[Any]:
        """
    ...
    def order_two_columns(self, column_boxes: List[Any]) -> Tuple[List[Any], List[Any]]:
        """
        Order bboxes within a single column based on y-coordinate
        Args:
            column_boxes (list): list of culumns of bounding boxes
    ...
    def order_horizontal_colums(self, column_boxes: Tuple[List[Any], List[Any]]) -> List[Any]:
        """
    ...
    def order_paragraphs(
        self, bounding_boxes: List[Any], img_size: Tuple[int, int], header_height: int = 0, footer_height: int = 0
    ) -> Tuple[List[Any], List[Any]] | List[Any]:
        """
        This fuction order paragraphs bounding boxes in a logical readable order taking in acount the number of columns
        detected, and the header and footer height
        Args:
            bounding_boxes (list): List of bounding boxes
        ...
        header_boxes: Tuple[List[Any], List[Any]] = [box for box in sorted_boxes if box[3] < header_height], []
        footer_boxes: Tuple[List[Any], List[Any]] = (
            [box for box in sorted_boxes if box[1] > (img_size[1] - footer_height)],
            [],
        )
    ...
    #  Docs assembly
    def crop_and_concat(self, image_path: str, bounding_boxes: List[Any], tag: str = 'vertical') -> str:
    ...
    def replace_from_extracted(
        self, text: str, tables_json_file_path: str, figures_json_file_path: str, equations_json_file_path: str
    ) -> str:
        """
    ...
    def load_pdf(
        self,
        pdf_path: str,
        output_folder: str = 'data/extraction',
        ...
        Args:
            ...
            save_intermediate (bool, optional): if is required to save some intermediuate results for debug purpouses.
            Defaults to False.
        ...
        for image_path in image_paths:
            ...
            mask_img_path, masked_img = self.mask_elements_from_image(
                masked_img, equations_bboxs, 'equation', save=True, image_path=file_path
            )
            # show structure bboxes
            _structured_bboxes_image = self.show_paddle_structure_bboxs(
                file_path, structured_ocr_result, save=save_intermediate
            )
            # clean up and order bounding boxes
            bboxes: List[Any] | Tuple[List[Any], List[Any]] | List[Any] = [
            ...
            assert mask_img_path is not None and isinstance(bboxes, list)
            ...
            full_text = self.replace_from_extracted(
                masked_text, tables_json_file_path, figures_json_file_path, equations_json_file_path
            )
