...
class CodeRAG(RAGComponents, CodeGenComponents):
    ...
    def build_rag_graph(self, workflow):
        ...
        workflow.add_edge("reformulate_query", "get_new_query") # Might need similar before code gen
        workflow.add_conditional_edges(
            "get_new_query",
            self.route_question,
            {
        ...
        # Placeholder for the hallucination CE.
        # Placeholder for the intermediate node.
        workflow.add_conditional_edges(
            "generate", # Move up as node to send to check hallucination
            self.determine_cont, # Make new intermediate pass node (raw_answer) to determination (replace generate)
            {
                "continue": "pass_to_codegen",
                "iterate": "detect_entities",
            },
        ) # Add a qd's answer node (replace raw_final_answer?)
        workflow.add_conditional_edges(
            "pass_to_codegen",
            self.route_question_to_code,
            {
                "llm": "return_raw_final_answer",
        ...
        {
            "executed": "return_raw_final_answer", #Change to final answer
        ...
        workflow.add_conditional_edges(
            "return_raw_final_answer",
            self.check_hallucinations, # Remove and place aboce as mentioned
            {
        ...
        workflow.add_edge("code_error_msg", "failure_msg")
