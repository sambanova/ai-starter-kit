...
class SambaNovaLLMClient(LLMClient):
    ...
    def llm_request(self, request_config: RequestConfig) -> tuple:
        ...
        metrics[common_metrics.NUM_TOTAL_TOKENS] = prompt_len + num_output_tokens
        metrics[common_metrics.REQ_OUTPUT_THROUGHPUT] = (
            num_output_tokens / (total_request_time) if total_request_time > 0 else None
        )
        return metrics, generated_text, request_config

    def _get_url(self, request_config: RequestConfig, base_url: str, project_id: str, endpoint_id: str) -> str:
        """Builds url for API
    ...
    def _get_total_request_time(self, url: str, headers: str, data: dict, stream: bool) -> tuple:
    ...
    def populate_server_metrics(self, output_data: dict, metrics: dict) -> dict:
        ...
        metrics[common_metrics.REQ_OUTPUT_THROUGHPUT_AFTER_FIRST_SERVER] = output_data["result"]["responses"][0].get("completion_tokens_after_first_per_sec")
        ...
        metrics[common_metrics.TOTAL_TOKEN_THROUGHPUT_SERVER] = output_data["result"]["responses"][0].get("total_tokens_per_sec")
        return metrics
