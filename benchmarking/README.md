...
- Configurable SambaStudio and SambanNova Cloud connectors. The connectors generate answers from a deployed model. You can use either of them to run OpenAI compatible endpoints from other providers (currently tested for **Fireworks** and **TogetherAI** endpoints)
...
**Note**: You can also use an OpenAI compatible endpoint from other provider like Fireworks, TogetherAI, etc. You will need to generate respective API keys on their platforms and map the URL and API Key to either SambaStudio or SambaNova Cloud environment variables.
