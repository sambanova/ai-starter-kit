...
# General parameters:
model_names = ['sarashina2-70b']  # add more models if necessary
llm_api = 'sncloud'  # it could be sncloud or sambastudio
results_dir = 'data/results/sarashina2_70b_2/'  # set the path where results will be saved

# Additional parameters:
timeout = 60000
sampling_params: Dict[str, Any] = {}
user_metadata: Dict[str, Any] = {}
time_delay = 120  # delayed time in seconds between runs

# Perf parameters:

# complete configs for sarashina2
# num_input_tokens = [512, 128, 2048, 128, 2000]  # add more input token numbers if needed
# num_output_tokens = [512, 128, 128, 2048, 2000]  # add more output token numbers if needed
# num_concurrent_requests = [1,2,4,8,16,32,64,128,256,512,1024]  # add more concurrent requests token numbers if needed

# configs for current run
num_input_tokens = [ 2048]  # add more input token numbers if needed
num_output_tokens = [ 128]  # add more output token numbers if needed
num_concurrent_requests = [1024]  # add more concurrent requests token numbers if needed
