#!/Users/jorgep/Documents/ask_public_own/ai-starter-kit-snova/.venev/bin/python3
...
if __name__ == '__main__':
    ...
    import xlrd
    from xlrd.timemachine import REPR, xrange
    ...
    def show_row(bk, sh, rowx, colrange, printit):
        ...
        if bk.formatting_info:
            for colx, ty, val, cxfx in get_row_data(bk, sh, rowx, colrange):
                if printit:
                    print('cell %s%d: type=%d, data: %r, xfx: %s' % (xlrd.colname(colx), rowx + 1, ty, val, cxfx))
        else:
    ...
    def bk_header(bk):
        print()
        print('BIFF version: %s; datemode: %s' % (xlrd.biff_text_from_num[bk.biff_version], bk.datemode))
        print('codepage: %r (encoding: %s); countries: %r' % (bk.codepage, bk.encoding, bk.countries))
        print('Last saved by: %r' % bk.user_name)
        print('Number of data sheets: %d' % bk.nsheets)
        print('Use mmap: %d; Formatting: %d; On demand: %d' % (bk.use_mmap, bk.formatting_info, bk.on_demand))
        print('Ragged rows: %d' % bk.ragged_rows)
        if bk.formatting_info:
            print('FORMATs: %d, FONTs: %d, XFs: %d' % (len(bk.format_list), len(bk.font_list), len(bk.xf_list)))
    ...
    def print_labels(sh, labs, title):
        ...
        for rlo, rhi, clo, chi in labs:
            print('%s label range %s:%s contains:' % (title, xlrd.cellname(rlo, clo), xlrd.cellname(rhi - 1, chi - 1)))
        ...
        for shx in range(bk.nsheets):
            ...
            if clabs or rlabs:
                if not hdr:
                    bk_header(bk)
                    hdr = 1
                print('sheet %d: name = %r; nrows = %d; ncols = %d' % (shx, sh.name, sh.nrows, sh.ncols))
    ...
    def show(bk, nshow=65535, printit=1):
        ...
        for shx in shxrange:
            ...
            print('sheet %d: name = %s; nrows = %d; ncols = %d' % (shx, REPR(sh.name), sh.nrows, sh.ncols))
            if nrows and ncols:
                # Beat the bounds
    ...
    def main(cmd_args):
        ...
        oparser.add_option('-l', '--logfilename', default='', help='contains error messages')
        oparser.add_option(
            '-v', '--verbosity', type='int', default=0, help='level of information and diagnostics provided'
        )
        oparser.add_option(
            '-m', '--mmap', type='int', default=-1, help="1: use mmap; 0: don't use mmap; -1: accept heuristic"
        )
        ...
        oparser.add_option('-s', '--onesheet', default='', help='restrict output to this sheet (name or index)')
        oparser.add_option(
            '-u', '--unnumbered', action='store_true', default=0, help='omit line numbers or offsets in biff_dump'
        )
        oparser.add_option(
            '-d', '--on-demand', action='store_true', default=0, help='load sheets on demand instead of all at once'
        )
        oparser.add_option(
            '-t', '--suppress-timing', action='store_true', default=0, help="don't print timings (diffs are less messy)"
        )
        oparser.add_option(
            '-r', '--ragged-rows', action='store_true', default=0, help='open_workbook(..., ragged_rows=True)'
        )
