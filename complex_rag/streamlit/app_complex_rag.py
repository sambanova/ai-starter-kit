...
from enterprise_knowledge_retriever.src.components.rag import ComplexRAG
from enterprise_knowledge_retriever.prompts.example_prompts import examples
 
CONFIG_PATH = os.path.join(kit_dir,'config.yaml')
LG_CONFIG_PATH = os.path.join(kit_dir,'lgconfig.yaml')
LG_PROMPTS = os.path.join(kit_dir,'lgprompts.yaml')
    ...
    with st.sidebar:
        ...
        if "Upload" in datasource:
            ...
            if st.button("Process"):
                with st.spinner("Processing"):
                    ...
                    # instantiate rag
                    rag = ComplexRAG(
                    config=LG_CONFIG_PATH,
                    prompts=LG_PROMPTS,
                    embeddings = st.session_state.embeddings,
                    vectorstore = st.session_state.vectorstore,
                    examples = st.session_state.examples)

                    lg_configs = rag.load_config(LG_CONFIG_PATH)
                    print(lg_configs) 
                    
                    # Initialize chains
                    # Replace with a call methods method.  for method in methods:
                        # get att(self, method()).  
                    rag.init_llm()
                    rag.init_router()
                    rag.init_example_judge()
                    rag.init_reform_chain()
                    rag.init_subquery_chain()
                    rag.init_retrieval_grader()
                    rag.init_entity_chain()
                    rag.init_qa_chain()
        ...
        with st.expander("Additional settings", expanded=True):
            ...
            if st.button("Reset conversation"):
                ...
                # Not great to reinitialize here
                rag = ComplexRAG(
                config=LG_CONFIG_PATH,
                prompts=LG_PROMPTS,
                embeddings = st.session_state.embeddings,
